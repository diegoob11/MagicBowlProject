...
public class PlayerController : NetworkBehaviour
{
    ...
    void Start()
    {
        ...
        temps_fin = count.GetComponent<Temps>();
    }

    void Update()
    {
        allowMove = temps_fin.allow_move_player;
        if (isLocalPlayer)
        {
            if (isDashing)
            {
            ...
            // Updates the speed value in the animator
    ...
    private void Move()
    {
        if (allowMove)
        {
            ...
            rigidBody.AddForce(mover.inputVector.x * movementSpeed, 0, mover.inputVector.z * movementSpeed);
